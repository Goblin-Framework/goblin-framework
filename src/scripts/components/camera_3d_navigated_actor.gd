...
var _players: Array[Actor3DNavigatedComponent]
...
func setup_camera_navigated_actor(camera: Camera3DNavigatedActorComponent, sensitivity: float):
	...
	setup_camera_base(camera, sensitivity, _navigation_actor)
...
func physics_process_camera_navigated(delta: float) -> void:
	physics_process_camera_base(delta)
...
func get_active_actor() -> Actor3DNavigatedComponent:
